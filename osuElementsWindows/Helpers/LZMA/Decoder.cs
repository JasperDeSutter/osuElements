...
namespace osuElements.Helpers.LZMA
{
    internal class Decoder
	{
        ...
        private readonly RangeDecoder _mRangeRangeDecoder = new RangeDecoder();
		...
		{
			_mRangeRangeDecoder.Init(inStream);
			...
			long inSize, long outSize, ICodeProgress progress)
		{
			...
			{
				if (m_IsMatchDecoders[state.Index << Base.kNumPosStatesBitsMax].Decode(_mRangeRangeDecoder) != 0)
					throw new DataErrorException();
				state.UpdateChar();
				var b = m_LiteralDecoder.DecodeNormal(_mRangeRangeDecoder, 0, 0);
			...
			while (nowPos64 < outSize64)
			{
					...
					// while(nowPos64 < next)
				{
					...
					if (m_IsMatchDecoders[(state.Index << Base.kNumPosStatesBitsMax) + posState].Decode(_mRangeRangeDecoder) == 0)
					{
						byte b;
						var prevByte = m_OutWindow.GetByte(0);
						if (!state.IsCharState())
							b = m_LiteralDecoder.DecodeWithMatchByte(_mRangeRangeDecoder,
								(uint)nowPos64, prevByte, m_OutWindow.GetByte(rep0));
						else
							b = m_LiteralDecoder.DecodeNormal(_mRangeRangeDecoder, (uint)nowPos64, prevByte);
						m_OutWindow.PutByte(b);
						state.UpdateChar();
						nowPos64++;
					}
					...
					{
						uint len;
						if (m_IsRepDecoders[state.Index].Decode(_mRangeRangeDecoder) == 1)
						{
							if (m_IsRepG0Decoders[state.Index].Decode(_mRangeRangeDecoder) == 0)
							{
								if (m_IsRep0LongDecoders[(state.Index << Base.kNumPosStatesBitsMax) + posState].Decode(_mRangeRangeDecoder) == 0)
								{
							...
							{
								uint distance;
								if (m_IsRepG1Decoders[state.Index].Decode(_mRangeRangeDecoder) == 0)
								{
									distance = rep1;
								}
								else
								{
									if (m_IsRepG2Decoders[state.Index].Decode(_mRangeRangeDecoder) == 0)
										distance = rep2;
									else
									{
										distance = rep3;
							...
							len = m_RepLenDecoder.Decode(_mRangeRangeDecoder, posState) + Base.kMatchMinLen;
							state.UpdateRep();
						}
						else
						{
							...
							rep2 = rep1;
							rep1 = rep0;
							len = Base.kMatchMinLen + m_LenDecoder.Decode(_mRangeRangeDecoder, posState);
							state.UpdateMatch();
							var posSlot = m_PosSlotDecoder[Base.GetLenToPosState(len)].Decode(_mRangeRangeDecoder);
							if (posSlot >= Base.kStartPosModelIndex)
							{
								...
								if (posSlot < Base.kEndPosModelIndex)
									rep0 += BitTreeDecoder.ReverseDecode(m_PosDecoders,
											rep0 - posSlot - 1, _mRangeRangeDecoder, numDirectBits);
								else
								{
									rep0 += _mRangeRangeDecoder.DecodeDirectBits(
									    numDirectBits - Base.kNumAlignBits) << Base.kNumAlignBits;
									rep0 += m_PosAlignDecoder.ReverseDecode(_mRangeRangeDecoder);
								}
							}
							else
								rep0 = posSlot;
			...
			_mRangeRangeDecoder.ReleaseStream();
		}
