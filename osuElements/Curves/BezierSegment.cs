    ...
    public class BezierSegment : CurveBase
    {
        ...
        public override Tuple<Position, float> GetPointOnCurve(float t)
        {
            ...
            if (t < 1)
            {
                ...
                return new Tuple<Position, float>(result, (Position.GetAngle(result - result2) + Constants.Math.PI2).NormalizeAngle());
            }
            result2 = GetBezierPointRecursive(t - Single.Epsilon, Points, 0, Points.Length);
            return new Tuple<Position, float>(result, (Position.GetAngle(result2 - result) + Constants.Math.PI2).NormalizeAngle());
        }
