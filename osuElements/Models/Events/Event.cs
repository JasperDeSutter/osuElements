...
namespace osuElements.Events
{
    public abstract class Event : IComparable<Event>
    {
        ...
        public static Event Parse(string s)
        {
            ...
            {
                string path = "";
                if (parts.Count() > 3) path = parts[3].Trim('\"');
                switch (etype)
                {
                    case (EventTypes.Sprite):
                        result = new SpriteEvent((EventLayer)Enum.Parse(typeof(EventLayer), parts[1]), (Origin)Enum.Parse(typeof(Origin), parts[2]), path, float.Parse(parts[4]), float.Parse(parts[5]));
                        break;
                    case (EventTypes.Animation):
                        Looptypes lt;
                        result = new AnimationEvent((EventLayer)Enum.Parse(typeof(EventLayer), parts[1]), (Origin)Enum.Parse(typeof(Origin), parts[2]), path, float.Parse(parts[4]), float.Parse(parts[5]), int.Parse(parts[6]), int.Parse(parts[7]), (Enum.TryParse(parts[8], out lt)) ? lt : Looptypes.LoopForever);
                        break;
                    ...
                    case (EventTypes.Background): //Background
                        result = new BackgroundEvent(int.Parse(parts[1]), parts[2], parts.Count() > 3 ? int.Parse(parts[3]) : 0, parts.Count() > 3 ? int.Parse(parts[4]) : 0);
                        break;
