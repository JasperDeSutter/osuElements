...
namespace osuElements.Other_Models
{
    public class PerfectCurve : CurveBase
    {
        ...
        public float Radius => Position.Distance(Center, Points[0]);

        double Circumference => Constants.Math.PI * 2 * Radius;

        public PerfectCurve(Position[] points) : base(points) { }
        protected override void Init() {
            Center = GetCenterFrom3Vectors(Points[0], Points[1], Points[2]);
            CalculateAngles();
            _length = Math.Abs(_curveangle) / Constants.Math.TAU * Circumference;
        }
        ...
        private void CalculateAngles() //used for calculating points
        {
            ...
            _curveangle = angle2;
            if (angle1 > angle2) {
                _curveangle -= Constants.Math.TAU;
            }
        }
        public override Tuple<Position, float> GetPointOnCurve(float t) {
            ...
            if (GetFreePoints(t, out free)) {
                return new Tuple<Position, float>(free, currentangle + ((_curveangle < 0) ? Constants.Math.PI : 0));
            }
            var x = (float)(Center.X + Radius * Math.Cos(currentangle));
            var y = (float)(Center.Y + Radius * Math.Sin(currentangle));
            return new Tuple<Position, float>(new Position(x, y), currentangle + ((_curveangle < 0) ? Constants.Math.PI : 0));
