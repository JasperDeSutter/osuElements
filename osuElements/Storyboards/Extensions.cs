...
namespace osuElements.Storyboards
{
    ...
    {
        public static string ToFileString(this TransformTypes transform) {
            if (transform == TransformTypes.MoveX || transform == TransformTypes.MoveY)
                return $"M{(char) transform}";
            ...
            if (floats.Length < 3)
                ...
                for (var i = 0; i < floats.Length - 1; i++) {
                    transformable.AddTransformation(new TransformationEvent(transformType, easing,
                        startTime + duration*i, endTime + duration*i, floats[i], floats[i + 1]));
                }
            }
        }
        ...
        {
            transformable.AddTransformation(TransformTypes.Color, startTime, endTime, easing,
        ...
        {
            transformable.AddTransformation(TransformTypes.Move, startTime, endTime, easing, values.Select(p => new[]{p.X, p.Y}).ToArray());
        }
        ...
        public static void Scale(this ITransformable transformable, int startTime, int endTime,
            Easing easing = None, params Position[] values)
        {
            transformable.AddTransformation(VectorScale, startTime, endTime, easing, values.Select(p => new[]{p.X, p.Y}).ToArray());
        }

        public static void Scale(this ITransformable transformable, int startTime, Position value)
        {
        ...
        {
            transformable.AddTransformation(TransformTypes.Scale, startTime, endTime, easing, values.Select(p => new[]{p}).ToArray());
        }
        ...
        public static void MoveX(this ITransformable transformable, int startTime, int endTime,
            Easing easing, params float[] values)
        {
            transformable.AddTransformation(TransformTypes.MoveX, startTime, endTime, easing, values.Select(p => new[]{p}).ToArray());
        }
        ...
        public static void MoveY(this ITransformable transformable, int startTime, int endTime,
            Easing easing, params float[] values)
        {
            transformable.AddTransformation(TransformTypes.MoveY, startTime, endTime, easing, values.Select(p => new[]{p}).ToArray());
        }
        ...
        public static void Rotate(this ITransformable transformable, int startTime, int endTime,
            Easing easing, params float[] values)
        {
            transformable.AddTransformation(TransformTypes.Rotate, startTime, endTime, easing, values.Select(p => new[]{p}).ToArray());
        }
        ...
        public static void Fade(this ITransformable transformable, int startTime, int endTime,
            Easing easing, params float[] values)
        {
            transformable.AddTransformation(TransformTypes.Fade, startTime, endTime, easing, values.Select(p => new[]{p}).ToArray());
        }
        ...
        {
            foreach (
                var pass in passed.Where(pass => MathHelper.Between(transform.StartTime, pass.StartTime, pass.EndTime))) {
        ...
        public static void OptimizeTransformations(this ITransformable transformable)
        {
            ...
            if (transforms[TransformTypes.Move].Any() && (transforms[TransformTypes.MoveX].Any() || transforms[TransformTypes.MoveY].Any())) {
                foreach (var m in transforms[TransformTypes.Move]) {
                    var mx = new TransformationEvent(TransformTypes.MoveX, m.Easing, m.StartTime, m.EndTime, m.StartValues[0].AsArray(),
                        m.EndValues[0].AsArray());
                    var my = new TransformationEvent(TransformTypes.MoveY, m.Easing, m.StartTime, m.EndTime, m.StartValues[1].AsArray(),
                        m.EndValues[1].AsArray());
                    transformable.AddTransformation(mx);
                    transformable.AddTransformation(my);
                }
                transformable.Transformations.RemoveAll(t => t.Transformtype == TransformTypes.Move);
