...
namespace osuElements.Storyboards
{
    public class SpriteEvent : Event, ITransformable
    {
        ...
        protected string ChildrenToString()
        {
            ...
            tostring.AddRange(Loopevents.Cast<TransformationEvent>());
            tostring.AddRange(Triggerevents.Cast<TransformationEvent>());
            tostring.Sort();
        ...
        protected string ActualToString() => 
            $"{Type},{Layer},{Origin},\"{Filepath}\",{StartX.ToString(Constants.Io.CULTUREINFO)},{StartY.ToString(Constants.Io.CULTUREINFO)}";
