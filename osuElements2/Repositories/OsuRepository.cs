...
using osuElements.Storyboard;
using static osuElements.Helpers.Constants.Io;
using Enum = System.Enum;

namespace osuElements.Repositories
{

    public class OsuRepository : IRepository<BeatmapBase>
    {
        ...
        public void ReadFile(string path, BeatmapBase result) {
            ...
            using (StreamReader sr = new StreamReader(path)) {
                ...
                if (line.StartsWith("osu"))
                    result.FormatVersion = Convert.ToInt32(line.Split(new[] { 'v', Splitter.Space }, removeEmptyEntries)[3]);
                else return;
                line = sr.ReadLine();
                ...
                while (line != null) {
                    ...
                    switch (section) {
                        #region General
                        case "[General]":
                            parts = line.Split(Splitter.Colon, removeEmptyEntries);
                            ...
                            switch (parts[0]) {
                                ...
                                case "SampleSet":
                                    ...
                                    break;
                                case "StackLeniency":
                                    result.StackLeniency = float.Parse(parts[1], FORMATPROVIDER);
                                    break;
                                case "Mode":
                                ...
                                case ("EditorBookmarks"): //only in first versions
                                    foreach (string bookmark in parts[1].Split(new[] { ',', ' ' }, removeEmptyEntries)) {
                        ...
                        case "[Editor]":
                            ...
                            switch (parts[0]) {
                                case ("Bookmarks"):
                                    if (parts.Length > 1) {
                                        string[] bms = parts[1].Split(new[] { ',', ' ' }, removeEmptyEntries);
                                        if (bms != null) {
                                ...
                                case ("DistanceSpacing"):
                                    result.DistanceSpacing = float.Parse(parts[1], FORMATPROVIDER);
                                    break;
                                case ("BeatDivisor"):
                                ...
                                case ("GridSize"):
                                    ...
                                    break;
                                case ("TimelineZoom"):
                                    result.TimelineZoom = float.Parse(parts[1], FORMATPROVIDER);
                                    break;
                            }
                            break;
                        #endregion
                        ...
                        case "[Difficulty]":
                            ...
                            switch (parts[0]) {
                                case "HPDrainRate":
                                    result.HPDrainRate = float.Parse(parts[1], FORMATPROVIDER);
                                    break;
                                case "CircleSize":
                                    result.CircleSize = float.Parse(parts[1], FORMATPROVIDER);
                                    break;
                                case "OverallDifficulty":
                                    result.OverallDifficulty = float.Parse(parts[1], FORMATPROVIDER);
                                    break;
                                case "ApproachRate":
                                    result.ApproachRate = float.Parse(parts[1], FORMATPROVIDER);
                                    break;
                                case "SliderMultiplier":
                                    result.SliderMultiplier = Convert.ToDouble(parts[1], FORMATPROVIDER);
                                    break;
                                case "SliderTickRate":
                                    result.SliderTickRate = float.Parse(parts[1], FORMATPROVIDER);
                                    break;
                            }
                            break;
                        case "[Events]":
                        ...
                        case "[TimingPoints]":
                            TimingPoint tp;
                            parts = line.Split(Splitter.Comma, removeEmptyEntries);
                            Timing offset = new Timing(Convert.ToInt32(float.Parse(parts[0], FORMATPROVIDER)));
                            float bpm = (float)Convert.ToDouble(parts[1], FORMATPROVIDER);
                            if (parts.Length > 2) {
                        ...
                        case "[Colours]":
                            ...
                            break;
                        case "[HitObjects]":
                            parts = line.Split(Splitter.Comma, removeEmptyEntries);  //Start making HitObject
                            ...
                            if (type.Compare(HitObjectType.Slider))                               //Make Slider
                            {
                                ...
                                var length = Convert.ToDouble(parts[7], FORMATPROVIDER);
                                s.Repeat = repeat;
                                s.Length = length;
        ...
        public string WriteToString(BeatmapBase t) {
            var output = new StringBuilder("osu file format v14" + NEW_LINE); //always save as newest version
            ...
            output.Append("//Background and Video events" + NEW_LINE);
            output.Append(t.Background == null ? "" : t.Background + NEW_LINE);
            output.Append(t.Video == null ? "" : t.Background + NEW_LINE);
            output.Append(t.BackgroundColorTransformations.Cast<Event>().Aggregate("", (current, e) => current + e.ToString()));
            output.Append("//Break Periods" + NEW_LINE);
            output.Append(t.BreakPeriods.Cast<Event>().Aggregate("", (current, e) => current + e.ToString()));
            output.Append("//Storyboard Layer 0 (Background)" + NEW_LINE);
            output.Append(t.BackgroundEvents.Cast<Event>().Aggregate("", (current, e) => current + e.ToString()));
            output.Append("//Storyboard Layer 1 (Fail)" + NEW_LINE);
            output.Append(t.FailEvents.Cast<Event>().Aggregate("", (current, e) => current + e.ToString()));
            output.Append("//Storyboard Layer 2 (Pass)" + NEW_LINE);
            output.Append(t.PassEvents.Cast<Event>().Aggregate("", (current, e) => current + e.ToString()));
            output.Append("//Storyboard Layer 3 (Foreground)" + NEW_LINE);
            output.Append(t.ForegroundEvents.Cast<Event>().Aggregate("", (current, e) => current + e.ToString()));
            output.Append("//Storyboard Sound Samples" + NEW_LINE);
            output.Append(t.SampleEvents.Cast<Event>().Aggregate("", (current, e) => current + e.ToString()));

            output.Append(title("TimingPoints"));
            output.Append(t.TimingPoints.Aggregate("", (current, tp) => current + tp + NEW_LINE));
            output.Append(NEW_LINE); // this is default stuff

            output.Append(title("Colours"));
            if (t.ComboColors.Any()) {
                for(int i = 0; i < t.ComboColors.Length; i++) {
                    output.Append($"Combo{i} : {t.ComboColors[i]}" + NEW_LINE);
                }
            }
            output.Append(t.SliderBorder == null ? "" : t.SliderBorder + NEW_LINE);
            output.Append(t.SliderBorder == null ? "" : t.SliderTrackOverride + NEW_LINE);

            output.Append(title("HitObjects"));
            output.Append(t.HitObjects.Aggregate("", (current, ho) => current + ho + NEW_LINE));
